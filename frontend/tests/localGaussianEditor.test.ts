...
for (const bound of [false, true]) test(`独立面板快速换源串行释放查看器和文档授权（绑定文档：${bound}）`, async () => {
  const hooks = createHookHarness(), exports: Record<string, any> = {};
  const opening = deferred<any>(), closing = deferred<void>(), authorizationClosing = deferred<void>();
  const authorizations: string[] = [], released: string[] = [];
  ...
  runInNewContext(code, { exports, AbortController, URL, console, document: { body: {} },
    ...
    require: (name: string) => {
      ...
      if (name === "./LocalGaussianSavePanel.tsx") return {};
  ...
  if (bound) {
    ...
    assert.deepEqual(authorizations, [b]); assert.deepEqual(released, [b]); assert.deepEqual(closed, [a]);
    authorizationClosing.resolve();
  }
