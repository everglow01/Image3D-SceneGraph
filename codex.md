...
- On 2026-08-12, an isolated COLMAP 4.0.0 learned-feature toolchain was added under ignored `external/colmap-4-cuda/`, built for RTX 4060 SM 89 with the versioned CUDA 12.2 compiler, ONNX Runtime 1.24.1, and cuDNN 9.25. It is selected only through the existing explicit `IMAGE3D_COLMAP_BIN` override; project-local COLMAP 3.9.1 with SIFT remains the production default. A temporary ALIKED_N16ROT + ALIKED_LIGHTGLUE CUDA smoke extracted 6,144 features from three synthetic images and produced 876/876 geometrically verified matches; a 10-image LightGlue check produced 6,464 verified matches across all 45 pairs in 7.10 seconds, while the explicit CPU control was killed after 20.20 seconds at 8.54 GiB RSS. The driver's missing optional `cuStreamGetGreenCtx` symbol warns under cuDNN 9.25 but does not prevent the measured CUDA path, so cuDNN was not downgraded. The preserved 3.9.1 CUDA SIFT smoke also passed. Installing the toolchain does not promote ALIKED, LightGlue, or Global Mapper: those require separate controlled Validation ablations, and Test remains unavailable for feature, matcher, mapper, threshold, or default selection. CUDA 11.7 and COLMAP 3.9.1 remain until both the fallback and Graphdeco's `cu117` extensions are migrated and deletion is separately approved.
...
- The frozen next-work priority is: (1) first-person scene traversal with keyboard movement, pointer-lock mouse look, real-time novel-view rendering, collision volume, explicit walkable boundaries, and no escape from the reconstructed scene—the exact collision/boundary contract is specified before implementation; (2) approximately two-minute multi-room apartment video reconstruction with quality-aware keyframe extraction, blur/duplicate rejection, robust cross-room loop closure, global pose optimization/bundle adjustment, bounded drift, and serial GPU execution; (3) 3DGS robustness and quality under difficult lighting, specular/transparent glass, and blurred glass; (4) batched VGGT-BA integration and controlled experiments; (5) resume Project trainer changes using the retained screen-pruning factorial evidence. Product work must not claim metric dimensions while the scene remains normalized arbitrary units.
