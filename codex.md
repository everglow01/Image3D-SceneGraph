...
- The bounded ALIKED positive-score round completed on 2026-09-07. The isolated candidate built from COLMAP `8bac7b9` without changing the production binary; build Job `20260907-134528-8881`, extraction smoke `20260907-135408-43f8`, and six-cell geometry Job `20260907-135825-3d54` all exited 0 after two preserved infrastructure failures (missing candidate ONNX runtime, then fail-hard `nvidia-smi` sampling). CPU/CUDA smoke proves all retained positive points preserve coordinates and descriptors exactly while weak frames shrink from about 4,096 to 827/436–437 points. In the 48-frame weak clip, original and filtered ALIKED both register 48/48 and pass catastrophic pose health; filtering halves feature input and reduces matching time 55.74%, but its scale-invariant camera max/median worsens from 3.05 to 40.21 at frame 712 and it creates a second 4-camera fragment. The original subset did not reproduce the full-video catastrophe, so this is neither a geometry fix nor a complete causal failure. In the healthy clip both register 48/48 and remain near 1.5 max/median, while filtered sparse points fall 13.47%. Do not promote the candidate, lower health thresholds from this one experiment, or run full video/Gaussian automatically. Complete evidence and hashes are in `docs/aliked-positive-score-experiment-20260907.md`; production assets/defaults remain unchanged. A subsequent Git-sourced read-only comparison (`a33b46a`, Job `20260907-152451-3bca`) maps every retained feature exactly back to the old index space. In the weak clip, 44.91% of original candidate and 43.17% of verified correspondences use at least one nonpositive endpoint, while retained verified Jaccard is only 77.16%; in the healthy clip those values are 2.90%, 2.57%, and 97.28%. Sim3-aligned camera-center max residual is 258.58% of original median radius in the weak clip versus 1.40% in the healthy clip. This establishes real weak-scene view-graph contamination and a LightGlue/input interaction, but neither solution is ground truth; filtering alone is not promotable.
...
- On 2026-09-02, the user paused Phase 4 until a bounded maintainability review could land. The review removes the `jobs -> adapters -> worker -> jobs` execution-layer dependency cycle, centralizes core file hashing and verified COLMAP asset installation, deduplicates stable/legacy pairing policy and frontend availability checks, deletes the unreachable automatic Test-evaluation branch while keeping `gaussian_test_status=not_run`, and establishes a locked Ruff baseline. Explicit setup `--install` may repair a corrupt project-managed model/tree only by validating a replacement `.part` before atomic publication; Jobs still never download assets. This is behavior-preserving infrastructure work except for that repair path: no reconstruction command, threshold, schema, default, Phase 4 feature, Test consumption, real experiment, or quality claim changes.
- On 2026-09-03, Phase 4 productized two-view geometric verification as `sfm_geometric_verification=default_v1|guided_v1` across ordinary COLMAP, COLMAP+VGGT, Project ordinary geometry, and the final COLMAP database stage of VGGT-BA. Both profiles explicitly keep geometric verification enabled and use the same COLMAP-build-default `TwoViewGeometry`/RANSAC parameters; experimental `guided_v1` changes only `FeatureMatching.guided_matching` from 0 to 1. Standard-v2 `matches_importer` recovery receives the same feature, local-matcher, and geometric-verification options as initial matching. SfM diagnostics schema 3 and pair schema 2 distinguish tentative candidates, candidates surviving geometry, Guided-added correspondences, final verified correspondences, and rejected candidates, and add a verified View Graph summary covering edges, components, degree, isolated/degree-1 nodes, video edge spans, and direct evidence across the existing soft gaps above two seconds. Historical schema 1/2 remains readable as `default_v1`, and `scripts/analyze_sfm_view_graph.py` derives the same summary without modifying an accepted Job or rerunning reconstruction. API/backend capabilities/frontend expose only the high-level profile and compact evidence, not raw RANSAC controls. `default_v1`, incremental Mapper, camera/triangulation/BA behavior, video gap softness, arbitrary-unit semantics, and Test isolation remain unchanged; no real Guided A/B, default promotion, Phase 5+ work, reconstruction, or training is claimed by this code landing.
- On 2026-09-03, Phase 5 productized camera model and intrinsic-sharing control as `sfm_camera_calibration=shared_opencv_v1|shared_simple_radial_v1|auto_grouped_simple_radial_v1`. Omitted requests preserve each backend's actual history: Project ordinary COLMAP and VGGT-BA use one shared OPENCV camera, while direct COLMAP and COLMAP+VGGT use one shared SIMPLE_RADIAL camera. The multi-image-only auto profile uses project-owned deterministic focal-aware grouping from normalized Make/Model, optional LensModel, focal/35mm-equivalent evidence, decoded dimensions, and EXIF Orientation; incomplete/invalid evidence becomes a per-image camera, and filename/order/directory/GPS/serial metadata is not used or retained. Repeated groups and singleton groups are batched through `image_list_path`, then their database partition and final image-to-camera assignments are verified. COLMAP+VGGT explicitly rejects OPENCV because current dense fusion cannot consume tangential distortion; VGGT-BA rejects auto-grouping, while standard-v2 recovery continues to inherit the unique existing camera. All current COLMAP-backed adapters require `sfm_camera_calibration_diagnostics`, generated from the raw sparse model before undistortion, with model/group/prior/registration/focal/principal-point/distortion/track/reprojection evidence. COLMAP-default plausibility bounds produce soft warnings only; non-finite focal, model/profile mismatch, shared-camera splitting, planned/database drift, and final assignment changes remain hard contract errors. SfM diagnostics schema 4 adds validated camera provenance and per-image camera IDs while schemas 1–3 remain readable as historical Project shared OPENCV. API/backend capability/frontend expose only high-level profiles and compact counts, not raw intrinsics. Phase 6 Mapper, Phase 7 triangulation/BA, product gates, arbitrary-unit semantics, Test isolation, and all prior algorithm defaults remain unchanged; no real camera A/B, reconstruction, training, quality gain, or promotion is claimed by this code landing.
- On 2026-09-03, failed ALIKED+LightGlue video Job `20260902_030611_a94e38dd` was classified as unusable downstream geometry rather than a learned-feature CUDA limit. COLMAP registered 1,000 images, but camera 61 was an isolated pose roughly 999× outside the remaining trajectory; the frozen mean/max normalization then collapsed the body and most 3NN scales hit `sqrt(1e-7)`. Native rendering compounded this by passing `camera_from_world @ world_from_normalized` with the raw `1.297e9` similarity scale to gsplat, making its 0.01 near plane operate in raw COLMAP units and admitting near-zero normalized depths with extreme tile radii; Project v7 OOMed in `isect_tiles` at iteration 2 before densification. Render cameras now remove the uniform dataset scale and explicitly use normalized-unit near/far planes. A CPU-only `gaussian_geometry_readiness_v1` gate writes robust camera/scale/projection evidence before CUDA and rejects the observed ≥100× median + ≥10× p99 isolated-camera shape or ≥50% 3NN-floor collapse; Project's existing max-world-scale pruning also applies before its first render, while MCMC/Graphdeco method semantics stay unchanged. `derive_gaussian_pose_repair.py` requires explicit image IDs and atomically creates a separately hashed `repaired_derivative`: filtered camera source/splits/tracks, renewed length-3 support, preserved parent sparse coordinates without a new BA claim, renewed normalization, 3NN initialization, replay, and parent→child provenance; it never overwrites the failed Job or starts training. The original arm remains `unusable_camera_pose_outlier`, downstream OOM, and inconclusive for ALIKED+LightGlue quality. Test RGB/metrics, model defaults, SfM controls, video profiles, arbitrary-unit semantics, and remote execution are unchanged; no real training or quality result is claimed by this fix.
- On 2026-09-03, the user explicitly promoted `video_keyframes_standard_v2` to the default for new video requests across API, frontend, direct `JobStore`, adapter fallback, and the standalone keyframe selector. The completed Project Job `20260828_020342_2b11a62c` is the product-run evidence: 1,824 initial selected frames, 107 recovery additions, 1,931 final selected frames, 1,742 registered frames (90.2%), full temporal coverage, and a completed training/export lifecycle. It passes the existing product gates of at least 12 registered frames, 70% registration, and 80% temporal coverage, but its four remaining gaps above two seconds and 90.2% registration do not satisfy the separately frozen 95%/zero-gap strict promotion evaluator; no evidence establishes its ≤2× timing condition either. The default change is therefore a user-directed override of that former promotion blocker, not a retroactive PASS or rewrite of historical evidence. Gaps remain soft warnings. Explicit `standard_v1` remains replayable, historical manifests are unchanged, and the switch does not promote ALIKED, LightGlue, sequential/vocab-tree pairing, Guided verification, a camera profile, Global Mapper, VGGT-BA, recovery-prune, MCMC, or any trainer/postprocessor setting. The newly exposed multi-camera SfM pose-health failure remains an open upstream problem; Gaussian readiness only prevents known catastrophic geometry from reaching native CUDA. Test remains excluded from selection and coordinates remain normalized arbitrary units.
- On 2026-09-04, the upstream multi-camera failure from `20260902_030611_a94e38dd` gained an SfM-layer root defense without relabeling the failed arm. `sfm_pose_health_v1` now checks every raw Project Gaussian sparse model before undistortion, normalization/split, initialization, or CUDA. Its hard reasons cover the observed isolated camera and p99-polluting multiscale branch; timestamp-normalized translation/rotation evidence, registration timeline, observed-depth/positive-depth support, bounded covisibility components, and database-backed healthy-core↔outlier bridge pairs remain diagnostic evidence. Ordinary COLMAP evaluates every incremental model and accepts only pose-healthy candidates meeting 12 registered cameras plus video 70% registration/80% temporal coverage. If none passes, it first copies SQLite, runs `view_graph_calibrator + global_mapper` on the same features/matches and frozen ≤1,000-frame v2 seed; only a failed Global attempt permits one video-only healthy-core repair, requiring a catastrophic timestamp boundary, removing the complete detected branch capped at 10%, filtering tracks to length 3, running BA, and reapplying all gates. V2 expansion/recovery/final BA reject any pose-regressing candidate and preserve the last healthy model; >2s gaps remain soft. `sfm_pose_recovery_v1` records requested/effective Mapper, relative paths, sparse-model/source-and-effective-database hashes, candidate health and removed IDs, while the adapter/manifest/frontend publish compact pose-health, solver, and recovery evidence. Global/core identities are recovered evidence, never clean incremental or ALIKED/LightGlue success, and production never silently switches SIFT/ALIKED, Brute-force/LightGlue, pairing, camera profile, trainer, or Test. `sfm_frontend_contract_v1` is written before Mapper so failed geometry arms remain comparable, and a JSON-only evaluator freezes the explicit SIFT/ALIKED × Brute-force/LightGlue 2×2 attribution boundary, including same-matches Global solver-sensitivity evidence. Local work is limited to CPU/mock tests; the old-Job read-only analyzer, a remote recovery geometry run, the real 2×2, and any subsequent Gaussian training each still require separate explicit user execution/authorization. Coordinates remain arbitrary units and historical Jobs/manifests are not rewritten.
- On 2026-09-07, the user authorized small-pair frontend verification and correction of the observed decision defects, with one remote L2 available. New raw reports use `sfm_pose_health_v2`: unchanged spatial limits plus an independent temporal discontinuity guard (at least 12 positive-speed steps, speed ≥100× positive-step p90 and displacement ≥median camera radius), and rejection of an empty sparse point cloud. This closes the dependency where a repaired 99.83× camera escaped both spatial and temporal checks; it does not lower spatial limits, expand automatic deletion, add repair rounds, or claim the remaining nine cameras are repaired. New adapters require v2, historical v1 assets stay immutable. `sfm_frontend_factorial_v2` selects the largest registered primary model (point-count/path tie-break), keeps product gates separate, treats absent pose evaluation as unknown, and rejects mixed health profiles. `analyze_sfm_small_matches.py` audits eight frozen bad/control pairs by image name, records exact keypoint/contract hashes and compares retained verified matches with SIFT-estimated epipolar geometry, explicitly not ground truth or official-reference equivalence. Local CPU verification passes 512 tests. Remote CPU audit Job `20260907-105158-ce45` completed against a hash-verified, isolated pre-Git snapshot: SIFT primary still passes; ALIKED-Brute-force primary has 81 independent discontinuities; ALIKED-LightGlue primary has 46 and the v1-repaired model has 13 and now fails; Global also receives `empty_sfm_point_cloud`. The eight-pair audit is complete. Subsequent Git-sourced single-L2 reference jobs completed on branch `sfm-small-match-audit` (`df48ea0`, `3d2d772`, `c79ad31`), after the first strict weight load identified an omitted deterministic confidence buffer. Official LightGlue on identical database features overlaps the stored COLMAP candidate sets by 98.33–100% Jaccard; two CPU ONNX controls match PyTorch sets exactly and all 120 directly named weight tensors agree. ALIKED frame 710 instead exposes 3,205 zero-score points among 4,032 valid-coordinate ONNX outputs, versus 786 official-reference detections: fixed TopK returns zero-score locations and the pinned C++ wrapper ignores the score output. Static ONNX inspection also establishes the constant-4096 TopK and 4096 upper clip. Two same-CPU-feature score-filter controls reduce weak-pair input from 4032/4032 to 791/399 but leave 22 matches unchanged, with no zero-score matched endpoint observed; this confirms unnecessary features, not a complete causal explanation or geometry-quality win. The production binary/assets remain unchanged and no full SfM/3DGS rerun is claimed. Evidence and remaining work are recorded in `docs/sfm-small-match-audit-20260907.md`. The user subsequently required all new code synchronization to use Git push/pull only; no further source-package transfer is permitted. No Gaussian training was started. The current 2×2 remains incomplete because SIFT-LightGlue OOM is not pose evidence.
