...
- Stage 5B research-only `vggt_ba` Gaussian geometry for video: fixed bounded VGGT windows, ALIKED/VGGSfM local tracks and BA, robust global Sim(3) window graph, then COLMAP 4 global triangulation/BA and Train-supported sparse initialization, with ordinary COLMAP remaining the default and no silent fallback;
