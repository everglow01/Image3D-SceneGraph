...
- `gaussian_trainer`: `project` (Project v7, default), `graphdeco` (Official comparison), or experimental `mcmc`; used only with `project_3dgs + gaussian_splat`
...
- `sfm_pairing`: multi-image `exhaustive` (default), video `sequential_loop` (default), or multi-image-only experimental `vocab_tree`
...
- `sfm_mapper`: `incremental` (default) or experimental `global`; explicit Global runs `view_graph_calibrator + global_mapper` on a database copy and never silently falls back to Incremental
...
- `gaussian_longest_edge`: 1280–3840px; used only with `project_3dgs + gaussian_splat`
...
- `geometry_backend=project_3dgs` with `output_type=gaussian_splat`, when COLMAP and the selected CUDA trainer are available. The default `gaussian_trainer=project` uses Project v7: the fixed `standard_v1` profile runs 30,000 iterations with a 1280px default and a frontend-selectable longest edge through 3840px, 3NN RMS initialization, training-time screen-radius pruning disabled, and Validation-selected export. Experimental `gaussian_trainer=mcmc` uses the installed Apache-2.0 gsplat MCMC strategy in the same native/distributed lifecycle, with the frozen `mcmc_v1` method package and a 3,000,000-Gaussian global cap. `graphdeco` invokes its pinned isolated research environment and remains an explicit Official comparison. Every trainer reuses the same geometry input and common SOR/Validation/export/manifest/Viewer path; MCMC is runnable but not promoted pending remote quality/resource evidence.
- `mode=video` with `project_3dgs + gaussian_splat`, when FFmpeg/ffprobe, COLMAP, and the selected trainer are available. Both profiles analyze one 10-second–15-minute MP4/MOV/M4V/WebM (906-second technical tolerance, up to 16 GiB) at 6 fps, bounded to 5,436 candidates. The default `video_keyframes_standard_v2` selects a 4 fps uniform base and motion-adaptive frames up to 5 fps: 3,600 base / at most 4,500 initial frames at 900 seconds, or 3,624 / 4,530 at the technical limit, before quality rejection. Ordinary-COLMAP v2 runs Mapper on at most 1,000 uniformly spaced base frames, then performs up to two `image_registrator` + non-clearing `point_triangulator` passes against the complete selected-frame feature database before gap recovery. This bounds Mapper cost without discarding the additional v2 views. Gap recovery runs at most two local COLMAP registration rounds with viable gap-bridging candidates from the remaining 6 fps pool. Each round is limited to 25% of the initial selection and recovery is limited to 50% cumulatively. The first accepted triangulation can feed a zero-new-candidate propagation round so new 3D points register deeper gap frames; rounds without strict gap improvement stop before expensive triangulation. Initial expansion and accepted recovery rounds share one final CUDA bundle adjustment with one CPU fallback. V2 also bounds intermediate Mapper global-BA frequency. Historical `video_keyframes_standard_v1` remains explicitly selectable and selects at most 1,000 upright frames without v2 expansion/recovery. The upload is staged in 8 MiB chunks; selected frames retain source PTS and generated JPEGs use EXIF Orientation 1 while authoritative metadata stays in JSON sidecars. Final registration must pass 12-frame, 70% registration, and 80% temporal-coverage gates before 3DGS starts; surviving gaps above 2 seconds remain a soft warning.

Video support is bounded offline reconstruction, not realtime SLAM or evidence of drift-free multi-room mapping. 4K H.265/HEVC MOV input is decoded by the installed FFmpeg build; no browser HEVC decoder or full-video pre-transcode is required. The existing 4096px / 12MP input limit covers UHD/DCI 4K. Frames are scored at 320×320 and selected frames are resized to the configured reconstruction longest edge (1280px by default), not automatically trained at 4K. Upload staging remains 8 MiB per chunk and source hashing is streamed; the larger input ceiling does not guarantee disk capacity, decode time, or GPU/host memory for a full reconstruction. HDR/log footage is not automatically tone-mapped or color-normalized. Coordinates remain normalized arbitrary units. FFmpeg and ffprobe are external executable dependencies; their absence disables only video ingestion, not image-based Project jobs.
...
Phase 3 exposes `sfm_pairing=exhaustive|sequential_loop|vocab_tree` independently from the local matcher. `sequential_loop` is the video default and uses temporal neighbors plus loop detection; `exhaustive` remains the multi-image default, while `vocab_tree` is multi-image-only and experimental. Both retrieval profiles require a descriptor-compatible tree under ignored `external/colmap-vocab/`: the 256K-word SIFT tree and official 64K-word ALIKED N16Rot tree are installed and verified separately. The setup script is dry-run by default; explicit `--install` atomically repairs a damaged managed tree after validating its replacement. Jobs never download or substitute trees:
...
The legacy video field `colmap_matcher=exhaustive|sequential` remains accepted and maps to `exhaustive|sequential_loop`; conflicting old/new fields fail. The production frontend defaults are `sift_v1 + bruteforce + sequential_loop + default_v1 + incremental` for video and use `exhaustive` for multi-image; camera defaults preserve the historical backend path: shared OPENCV for `project_3dgs` and shared SIMPLE_RADIAL for direct `colmap`/`colmap_vggt`. The 2026-08-13 2048-keypoint/1280px ETH3D run found SIFT-LightGlue nearly disconnected while ALIKED-LightGlue completed but cost substantially more matching time; those historical settings do not select the current 8192-keypoint default. Phase 3 pairing code is available but has no promotion-grade real A/B yet. Phase 6 exposes experimental `sfm_mapper=global` for ordinary COLMAP reconstruction while retaining Incremental as the default. Explicit Global is a primary solver with no silent fallback; the existing `global_recovery_v1` remains a distinct automatic same-matches recovery only after every requested-Incremental candidate fails pose health, and standard-v2 registration expansion remains explicitly incremental. VGGT-BA does not use this selector. Test cannot select these options. Graphdeco setup uses PyTorch `2.3.1+cu121` and compiles its extensions with `/usr/local/cuda-12.2`, while Project uses the existing `gsplat` cu121 wheel.
...
COLMAP output is a sparse SfM reference: it estimates a global camera graph and sparse point cloud. Use it to compare whether VGGT multi-image drift is caused by windowed model inference or by weak image overlap / texture. Video extraction writes `diagnostics/video_keyframe_timing.json`; ordinary-COLMAP v2 also writes `diagnostics/video_initial_registration_expansion.json` and `diagnostics/colmap_timing.json`. After same-source v1/v2 geometry-only runs and a second deterministic v2 extraction, `scripts/evaluate_video_v2_promotion.py` evaluates the frozen 95% registration, zero `>2s` gap, camera/point retention, recovery-budget, determinism, and `≤2×` time gates. A failed check still exits nonzero as strict diagnostic evidence, but the user-directed 2026-09-03 v2 default switch supersedes it as a product-default blocker and does not relabel historical failures as passing.
