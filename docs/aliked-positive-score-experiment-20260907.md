...
去除占位点、数值索引正确、减少计算输入、改善几何是不同结论。本轮前两项成立，也确认非正分点真实污染了弱区 view graph；几何改善和产品可用性没有同时成立。LightGlue 正分版注册完整但有 40.21× 软位姿异常，Brute-force 正分版轨迹紧凑但产品门槛失败；原版在两种 matcher 下的表面覆盖又大量依赖非正分端点。SIFT 是场景支持对照，不是 ground truth。生产二进制、ONNX、profile 与默认值保持不变；本阶段不再追加局部 matcher A/B。下一阶段若继续，应先设计版本化的正分 profile 与“可靠支持不足”处理，再用更大但仍有界的同源时间上下文验证；不能直接重跑千帧或 Gaussian。
