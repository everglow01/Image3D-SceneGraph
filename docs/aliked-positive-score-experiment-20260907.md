...
生产二进制、ONNX、profile 与默认值保持不变；本阶段不再追加相同低帧率输入的 matcher A/B，也不修改健康阈值。下一步先只读核对原 6 fps candidate 是否仍保留、三个断层内是否有更密且不同内容的可用帧；只有存在额外输入证据时，才值得设计高密局部恢复实验。否则应转向新的特征来源或明确失败，而不是直接重跑千帧、Gaussian，或用无效点填满覆盖率。
