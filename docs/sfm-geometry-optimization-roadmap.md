...
- 本地只做 CPU/mock 自动测试。原失败 Job 的 read-only analyzer、远端恢复路径和同源四格已运行，结果与未完成项见本节 2026-09-07 更新；后续实验不得以代码接入代替真实结论。
