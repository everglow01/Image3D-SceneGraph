...
2026-09-26 本地编辑P4补充：产品增加实验性本地编辑入口，复用Spark、源绑定CPU文档与导出，不创建云渲染／ICE／offer。云端在途操作或活跃会话须用户先明确关闭，才允许切模型／本地；不后台抢占，不改旧云协议和部署默认。代码与静态检查完成，回归／真实浏览器／GPU未运行，未push/pull/部署。见 [本地P4](gaussian-local-editor-p4.md)。

2026-09-24 本地编辑P3补充：独立前端组件接入P2授权／保存／CPU导出，不创建云渲染或媒体连接，不抢占云会话；历史版本mask读取仍需本地单文档授权，查看不写当前revision。仅代码与静态检查，新增回归未运行，无push/pull/部署或服务重启，旧云安全门禁不变。见 [本地P3](gaussian-local-editor-p3.md)。
...
日期：2026-09-21，修订于2026-09-22。**云修剪可用性修复已部署，双模型真实GPU服务端验收通过；新版真实产品浏览器交互及端到端画质/延迟仍未验收。**
...
## 云修剪可用性修复（2026-09-22，双模型服务端GPU验收通过）
...
- 默认可见贡献选择在新GPU适配器中前向累计alpha/透射并按可信前层阈值保守选源ID，低置信半透明区域不向背景扩张；已通过两模型单视角真实GPU选择/预览/删除撤销验收，**但不能保证后景零误删或跨视角画质**。旧中心/保守范围选择仅在明确的深度薄层或穿透模式下使用；本次会话保护选集最终排除于预览/删除。显式预算失败不回退穿透，不修改Original或持久编辑schema。
- 本地相关45项Python CPU/mock和前端76项通过，构建通过（既有大包警告）；本地Chrome真实页面在只读GET代理、mock会话条件下验证宽视口、高清准备、真实鼠标矩形自动计算、resize后仍可选、隔离预览后才允许删除，报告 `outputs/analysis/cloud-editor-local-ui-20260922-v2/result.json`。这仅是本地UI/CPU验证，不能代替下述真实GPU服务端验收，也不是新版真实浏览器视频路径的通过证据。

- **远端首轮真实GPU结果未通过，不作为可用验收**：隔离目录 `outputs/experiments/cloud-trim-acceptance-20260922-v1/` 的Project1,483,682模型加载/640×360 PNG成功，但三个矩形的默认可见选择均为0，未执行删除；报告exit1，源SHA不变、GPU与租约释放。只读单模型诊断 `outputs/experiments/cloud-trim-diagnostic-20260922-v2/` exit0：每个ROI有17k–32k真实Gaussian像素贡献，27个深度批次，但首批弱前层覆盖上限仅0.023–0.035，容差0.02/0.1/0.3均无法达0.5；定位为过早把弱前层视作必须选的表面并停止，不是没有贡献索引。新的CPU修复跳过累计<5%的孤立弱前层并寻找主表层；5%–50%仍放弃。失败v1与只读诊断v2原样保留，不把诊断exit0冒充功能通过。
- **修正后v3服务端GPU验收通过**：`cfa7c98` 主体修复、`580f32d` 验收脚本、`b88ded3` 诊断及 `4947fb3` 弱前层修正经Git同步，远端main为`4947fb3`；仅项目后端重启，TURN未重启。独立目录 `outputs/experiments/cloud-trim-acceptance-20260922-v3/`，任务 `20260922-110135-3206` 与独立只读监控 `20260922-110151-d069` 均exit0；本地小报告 `outputs/analysis/cloud-trim-acceptance-20260922-v3/result.json`。Project 1,483,682个高斯加载21.13秒、640×360固定帧render 561.23ms；32×32中心框默认可见选448、显式穿透选7,950。MCMC 2,999,158个高斯加载34.71秒、固定帧render 575.58ms；同框默认可见选6,422、穿透选18,197。两者均经会话保护、隔离及删除后预览，分别删除448/6,422再撤销恢复原数，关闭会话；单模型总耗时24.32/39.57秒，未保存/导出/训练/使用Test。两源SHA256前后不变，测试文档revision2、cursor0；结束时无CUDA compute、共享锁可取得、项目后端/TURN active、远端HEAD `4947fb3`。最后一次仅为只读文档复核，`.render-scratch/` 不含 `edit.json`，过滤后两测试文档状态正常，不是验收失败。
- 删除后预览相对原固定帧，阈值为单像素最大通道差>8：Project框内180/1024像素变化、框外58像素（0.025%）；MCMC框内526/1024、框外392像素（0.171%）。**框外变化非零**，不能宣称可见模式保证后景绝对不受影响；尚无多视角质量定量或用户原浏览器实操验证。

## 剩余验收边界

用户经公司SDP远程办公；TCP-only产品修复已部署，旧版独立Chrome短时联网复测不修改ICE配置，UDP仍不可达；用户原浏览器状态仍待实际反馈。新版默认室内参考视角已实现，但真实产品浏览器复测因远端资源预检权限受阻暂缓；不能把本地mock页面及服务器直接API的v3验收拼成浏览器端到端通过。
