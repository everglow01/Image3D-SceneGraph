...
- per-run `feature` (profile/extractor/descriptor/keypoint budget/extractor-model hash), `local_matcher` (stable `bruteforce|lightglue` profile, descriptor-compatible COLMAP enum and optional model hash), `pairing`, geometric-verification, Mapper and COLMAP-build provenance, plus job-relative feature-index and pair-index paths.
