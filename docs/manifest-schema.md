...
| `sfm_pairing` | string | Optional requested image-pair policy: multi-image `exhaustive` (default), video-only `sequential_loop` (default), or multi-image experimental `vocab_tree`. Historical manifests may omit it. |
...
| `sfm_mapper` | string | Requested SfM primary solver: default `incremental` or experimental `global`. Historical manifests may omit it and retain their recorded effective/recovery evidence. |
| `sfm_mapper_effective` | string or null | Effective primary/recovery identity: `incremental`, explicit `global`, automatic `global_recovery_v1`, or `incremental_core_repair_v1`; queued jobs use `null`. Explicit Global cannot silently become another identity. |
...
| `gaussian_trainer` | object | Optional selected trainer identity, label, pinned revision, and license. Current IDs are default `project`, explicit Official comparison `graphdeco`, and experimental native `mcmc`; historical manifests may omit it. |
...
| `gaussian_recovery_prune` | string | Optional recovery-gated prune request for project Gaussian jobs: `on` or `off`. Omitted Project v7 requests default to `on`; Graphdeco/MCMC default to `off`, and MCMC rejects `on`. The effective Gaussian config records the resolved `opacity_reset.recovery_prune` leaf. Absent on historical manifests. |
| `gaussian_final_fit` | string | Optional delivery refinement: `off` (default) or experimental `train_validation_v1`. The latter is available only for native Project/MCMC, starts after held-out Validation selection and SOR, optimizes Train+Validation with frozen topology, and never loads Test RGB. |
| `gaussian_final_fit_status` | string | `disabled`, `pending`, or `available`. An explicitly requested final-fit failure fails the Job rather than silently publishing the pre-fit source model. |
...
New successful COLMAP-backed jobs also retain the existing `colmap/` workspace directory at the job root by rename, including the feature/match SQLite database, raw binary/text models, recovery databases/models and undistorted sparse data. This is internal reconstruction evidence, not a new manifest asset: root `colmap/` is excluded from the job ZIP and asset endpoint. Existing public sparse-cloud/camera/diagnostic roles and the native training replay are unchanged. An interrupted publication quarantines a moved `colmap/` alongside the other attempt outputs before retry. Retention does not copy/recompute geometry or clean intermediate models, so completed jobs require more disk space. Historical jobs whose workspace was already deleted cannot recover these files through this change.
...
### Read-only Gaussian comparison

`result_kind=gaussian_comparison` identifies a separately published viewing container with at most four models, not a successful training Job. `default_gaussian_variant` names one member of `gaussian_variants`; each member has a unique filename-safe `id`, `label`, `trainer=project|mcmc`, `model_stage=selection|final_fit|train_only`, contained job-relative `scene_splat`/`export_metadata`, `model_sha256`, positive `gaussian_count`, and finite `psnr`/`ssim`/`display_psnr`/`display_ssim`. Selection uses `metric_role=held_out_model_selection`; Train+Validation final-fit uses `in_sample_after_train_validation_fit`; Train-only uses `held_out_after_train_only_control` (Validation was not used in the extra optimization, but remains development evidence, not Test). Metrics describe native renders, not the browser screenshot. The default asset roles must agree with the selected variant, and all variants share the hash-verified dataset normalization and camera path. Historical manifests without this list retain Original/VGGT-filtered viewing.

`scripts/publish_gaussian_final_fit_comparison.py` verifies the frozen experiment protocol, source/final/evaluation hashes, exact Validation IDs, roles, and unchanged Gaussian counts before atomically publishing a new directory. It does not train, hash/decode RGB, copy model snapshots, or create canonical/bundle duplicates. Its staging is outside the visible job list. Comparison results expose no navigation/mesh generation, cancel/retry, or whole-Job ZIP; existing source Jobs and experiments are immutable. The frontend uses one viewer and reloads/resets its camera on variant switching; synchronized-camera or pixel-equivalent browser comparison is not claimed.

`scripts/publish_video_train_only_comparison.py` instead consumes the completed two-arm `run_video_4k_comparison.py` experiment. It requires the expected protocol SHA256, verifies both completion/stage records, 30k/two-rank training and 2k/4k-sample Train-only identities, native model/export/evaluation hashes, exact Validation IDs and shared camera path. It hardlinks the two existing final PLYs on the same filesystem and copies only small viewing/evidence JSONs into a new atomic publication; Project v7 is initially selected and MCMC is the second variant. It never loads model tensors or any dataset RGB, re-exports a PLY, fabricates training attempts, or changes source records. Existing output IDs are rejected and failed staging is retained outside the visible list. This publisher requires the frontend's `train_only` variant support; do not publish it to an older page under a false Selection or Train+Validation role.
...
- `gaussian_selection_model`, `gaussian_final_fit_record`, `gaussian_final_fit_evaluation`, and `gaussian_final_fit_progress` only when experimental Train+Validation final-fit runs; `gaussian_evaluation` remains the pre-fit held-out selection report while `gaussian_model` and `scene_splat` become the final-fit delivery model
...
Navigation failure logs are diagnostic only: initial job generation appends a timestamped traceback and captured subprocess stdout/stderr to `diagnostics/navigation_failure.log`; later navigation attempts use `lifecycle/navigation/attempt-NNN/failure.log`. No failed navigation assets are published, and logging failures do not turn a successful reconstruction into a failed job. Navigation algorithms, quality gates and retry behavior are unchanged.
...
When final-fit is available, `gaussian_final_fit_*` metrics record profile/hash, optimizer updates, camera samples, elapsed time, world size, fixed Gaussian count, and source/final model hashes. `gaussian_final_fit_fit_psnr`/`ssim` plus display variants use the existing metric profiles but describe `fit_validation` with `quality_role=in_sample_after_train_validation_fit` and `selection_eligible=false`; they must never replace or be compared as held-out `gaussian_validation_*`. Export metadata adds a `final_fit` lineage block and the result bundle carries `final-fit/record.json`; when SOR is present its record remains bound to the final-fit source model. `train_validation_v1` is initially incompatible with `vggt_visibility_v1` so that derivative's Train-only decision and held-out reporting semantics cannot be misrepresented.
...
质量优先策略（2026-09-23 后续修订）：旧查看器默认加载原始 PLY；已发布 K2 仅作为显式手动选项，标注「有损压缩，画质待验收」，未通过固定相机画质验收前不自动启用。K2 等完整下载/构建成功才 ready；加载/格式错误释放实例后只自动回退一次 PLY，取消/切模型和 metadata 错误不触发回退。仍保留显式隔离 A/B 查询入口。同模型资产切换保持视角，换模型重新默认 PLY；Spark 与云端查看/编辑继续用原始来源，CPU 距离计算保持默认。`scene_splat`、canonical、模型、训练配置、导出包、历史指标和导航合同不变；K2 的原始高斯数不变，但有量化损失，不能宣传为无损、减点或保证 FPS。旧查看器0.4.7读取原始SH3 PLY时也最多有效SH2，因此默认PLY只是避免额外K2量化，不能代表完整SH3渲染；有效阶数继续在界面显示，不自动切换实验渲染器。

Job asset API 对 `.ply`（包括普通点云）和 `.ksplat` 使用 `Cache-Control: private, no-cache`：允许浏览器私有缓存，但复用前必须向服务器重验证。先执行既有路径/文件检查，再按文件stat生成的ETag及Last-Modified处理条件GET/HEAD，匹配时返回无body的304；ETag优先于日期条件，未匹配保持完整GET和Range/If-Range语义。没有把未版本化路径设为长期immutable缓存，不在请求中重新哈希大文件。缓存是否实际保留大型模型取决于浏览器容量/策略，须用真实暖加载的传输字节数验收；304支持不是首次加载提速或完整模型缓存命中的证明。其他JSON、动态元数据和实验资产路由策略不变。

原始Gaussian PLY写入改用NumPy按C顺序直接写文件，ZIP条目以8 MiB块流式读取，避免额外整模型bytes副本；字段、float32小端布局、全部SH3数据、文件名、排序、ZIP时间戳与压缩算法不变。逐字节回归覆盖旧写法和新写法的PLY/ZIP输出，原始模型/历史产物不回写；内存与耗时改善仍需实测，不将减少复制直接标为已测加速。
...
`sfm_pose_health` uses schema 1 / `sfm_pose_health_v2` for new runs; historical `sfm_pose_health_v1` assets remain readable and are never rewritten. It is evaluated on the final raw COLMAP text model before undistortion, split construction, Gaussian normalization, or CUDA. It records camera-center distributions, observation/depth support, covisibility, spatial outliers and bridge pairs as before. V2 preserves the v1 spatial thresholds and adds `empty_sfm_point_cloud` plus `temporal_pose_discontinuity` to the existing `isolated_camera_pose_outlier`, `multiscale_camera_pose_branch`, and `degenerate_camera_extent` hard reasons. The independent temporal policy requires at least 12 positive-speed steps, a step speed ≥100× the p90 of all positive step speeds, and a displacement at least the median camera distance from the robust center. It does not require either endpoint to be a spatial outlier. `temporal.discontinuity_count` is untruncated, `discontinuities` contains the 32 highest-speed-ratio records, and the positive-speed baseline and policy are explicit. Rotation alone, short local fast steps, and registration gaps do not trigger this new hard reason. The original spatially bounded repair set and one-attempt/10% cap are unchanged: temporal-only failures cannot authorize extra deletion, and a repaired model with residual discontinuities is rejected. New runner results must report v2; an old v1 PASS is not sufficient for a new adapter run. Coordinates remain arbitrary world units, Test RGB is not loaded, and a final report must be `passed` before proceeding.

Ordinary-COLMAP jobs also publish schema 1 / `sfm_pose_recovery_v1`. Requested Incremental checks every Mapper model before selection; if none is healthy, the runner copies the SQLite database, runs `view_graph_calibrator` and calibrated Global Mapper with the same feature/match/pairing/calibration identity, and accepts it only after the same pose and 12-frame/70%-registration/80%-temporal-coverage gates. Only when that automatic Global recovery fails may a timestamped video remove one complete catastrophic branch, capped at 10% of registered cameras, then run point filtering and bundle adjustment before all gates are reapplied. Explicit experimental Global instead runs calibration and Global Mapper as the primary solver on its own database copy; healthy Global candidates are primary evidence with `requested_mapper=effective_mapper=global`, `status=not_needed`, and `recovery_applied=false`. Explicit Global failure never invokes Incremental or core repair. Effective identities remain distinct: `incremental`, explicit `global`, automatic `global_recovery_v1`, or `incremental_core_repair_v1`. The record contains job-relative candidate/model/database paths, sparse-model file hashes, source/effective database hashes, removed image IDs, and failure-safe provenance. No path silently switches feature, matcher, pairing, camera profile, or requested primary solver.

For video Gaussian jobs only, an initial Incremental Mapper `SIGABRT` with the exact zero-image global-BA assertion may proceed to this same candidate-validation/recovery chain when saved binary candidates exist. This is not a successful Mapper execution: `mapper_failure` records `incremental_empty_global_ba_abort_v1`, return code, original log hash and saved-file hashes. A selected Incremental candidate after this abort uses `status=recovered_after_mapper_abort` and `recovery_applied=true`; automatic Global/core retain their distinct effective identity and `recovered` status. All candidates still require complete model parsing, pose health and the unchanged registration/coverage gates. Missing candidates, unknown crashes, OOM, cancellation and explicit Global failures remain failures. Raw failure logs stay in diagnostics, not public manifest assets.

`sfm_mapper` defaults to `incremental`. Explicit `global` is experimental and is available only where ordinary COLMAP constructs the sparse camera model: direct COLMAP, COLMAP+VGGT without `--colmap-model-dir`, and Project Gaussian with `gaussian_geometry_source=colmap`. VGGT-BA and reused external text models reject it because no selected Mapper runs there. Global uses fixed COLMAP-build defaults plus seed 0 and requested GPU/thread/image-list controls; raw GLOMAP parameters are not public. For standard-v2 video, the Global primary may consume the same bounded seed image list, while later `image_registrator`/triangulation expansion remains separately identified as incremental and does not rewrite the effective primary solver.

`scripts/run_colmap_sparse.py` writes `diagnostics/sfm_frontend_contract.json` (`sfm_frontend_contract_v1`) before Mapper execution so a pose-failed standalone arm still retains its frozen COLMAP build, feature, local matcher, pairing, geometric verification, camera profile, video-selection hash, COLMAP default random seed `0`, Mapper policy, and v2 seed contract. `scripts/evaluate_sfm_frontend_factorial.py` consumes four existing geometry-only arms (SIFT/ALIKED × Brute-force/LightGlue), rejects mismatched contracts, and reports only bounded extractor/matcher/interaction/common-pipeline or solver-sensitivity evidence. It never runs geometry or training. The `sfm_frontend_factorial_v2` evaluator uses the primary model with the largest registered count, then point count, then lexical model path; it records that choice explicitly rather than allowing a two-camera fragment to make an unhealthy main reconstruction pass. Product acceptance remains separate. Missing/failed-to-evaluate pose evidence is unknown, not a failed-pose observation, and produces an inconclusive report.
...
## Gaussian recovery-gated prune

```text
gaussian_recovery_prune = on | off
Project default = on; Graphdeco/MCMC default = off
```

Resolution order is explicit request, then `IMAGE3D_GAUSSIAN_RECOVERY_PRUNE`, then the trainer-aware default. Project v7 `on` enables the schema-10 `opacity_reset.recovery_prune` leaf, and the trainer fires a one-shot prune of gaussians still below the recovery threshold 500 iterations after each opacity reset (progress events carry `recovery_prune` with before/after counts). The request is echoed as the `gaussian_recovery_prune` manifest field, and the effective leaf is visible in the `gaussian_config` record. Explicit Project `off` and the environment kill switch remain available. Graphdeco does not implement this Project strategy; MCMC disables it and rejects explicit `on`. The user-directed 2026-09-08 default promotion builds on the passed room A/B and prior video execution evidence without treating the missing strict video on/off A/B as completed causal evidence. No frontend switch exists yet.
