...
`result_kind=gaussian_comparison` identifies a separately published four-model viewing container, not a successful training Job. `default_gaussian_variant` names one member of `gaussian_variants`; each member has a unique filename-safe `id`, `label`, `trainer=project|mcmc`, `model_stage=selection|final_fit`, contained job-relative `scene_splat`/`export_metadata`, `model_sha256`, positive `gaussian_count`, and finite `psnr`/`ssim`/`display_psnr`/`display_ssim`. Selection uses `metric_role=held_out_model_selection`; final-fit uses `in_sample_after_train_validation_fit`. Metrics describe native renders, not the browser screenshot. The default asset roles must agree with the selected variant, and all variants share the hash-verified dataset normalization and camera path. Historical manifests without this list retain Original/VGGT-filtered viewing.
