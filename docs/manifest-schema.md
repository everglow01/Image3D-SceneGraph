...
- `video_probe`, `video_frame_selection`, `video_keyframe_timing`, `video_registration_diagnostics`, and `video_keyframe_contact_sheet` for a completed bounded video attempt; explicit ordinary-COLMAP `standard_v2` attempts additionally publish `video_registration_recovery` and `colmap_timing`
