...
`sfm_diagnostics` schema 3 (`profile=sfm_frontend_diagnostics_v3`) records:
...
Metrics on success include the existing diagnostics/image/feature/local-matcher/pairing/Mapper fields plus `sfm_geometric_verification_profile`, `sfm_view_graph_verified_edge_count`, `sfm_view_graph_component_count`, `sfm_view_graph_largest_component_ratio`, `sfm_view_graph_isolated_node_count`, and `sfm_view_graph_guided_inlier_count`. Full graph distributions stay in the diagnostics asset rather than being duplicated into top-level metrics.
